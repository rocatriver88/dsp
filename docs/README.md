...
## API 契约
- `swagger.yaml` / `swagger.json`：生成的 OpenAPI 2 产物
- `openapi3.yaml`：前端类型生成流程使用的 OpenAPI 3 契约
- `docs.go`：生成的 Go swagger 元数据

如果修改了 API handler 或 swagger 注解，请执行 `make api-gen` 重新生成这些文件。

## 验证资料
- `browse-reports/`：浏览器验证报告和截图
- `superpowers/plans/`：实现计划
- `superpowers/specs/`：设计和范围文档
