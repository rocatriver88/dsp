...
> **实现状态**
> - Campaign 路由的 publish 已在 commit `becdc67` 落地。
> - Creative 路由的 3 个 publish **当前尚未实现**,属于本轮 **biz QA 的修复范围**(见 §4 责任边界)。实现要点:`HandleCreateCreative` 从请求 body 的 `campaign_id` 取;`HandleUpdateCreative` / `HandleDeleteCreative` 需先 `GetCreative` 拿到 `CampaignID` 再调 `NotifyCampaignUpdate`,避免"删了还广播广告 ID"的悖论。
...
| Postgres 广告主/广告/创意 CRUD | ✅ 正确性 + 发 pub/sub(含本轮要修的 creative CRUD 3 条) | 读(作为前置条件) |
